...
import type { TemporalSnapshot } from '@/core/temporal';
...
import { DEFAULT_AUTOPOIETIC_CONFIG } from '@/organism/autopoietic/L16_autopoietic_layer';
...
const autopoieticConfig = DEFAULT_AUTOPOIETIC_CONFIG;
...
let lastAutopoieticSnapshot: AutopoieticSnapshot | undefined;
...
const describeOntogenesisVector = (params: {
  ...
  autopoieticSnapshot?: AutopoieticSnapshot | null;
  note?: string;
}): OntogenesisVector => {
  ...
  const autopoieticSnapshot = params.autopoieticSnapshot ?? lastAutopoieticSnapshot ?? undefined;
  ...
  return {
    ...
    autopoietic: autopoieticSnapshot ?? vector.autopoietic,
  };
};
...
const temporalHistory: TemporalSnapshot[] = [];
...
heartbeat.onBeat((beat) => {
  ...
  const baseOntogenesisVector = describeOntogenesisVector({
    ...
    autopoieticSnapshot: lastAutopoieticSnapshot,
  ...
  const organismSnapshot = runOrganismPipeline({
    ...
    previousAutopoietic: lastAutopoieticSnapshot,
    autopoieticConfig,
    temporalHistory: temporalHistory.slice(-8),
    now: Date.now(),
  });
  ...
  lastAutopoieticSnapshot = organismSnapshot.autopoietic ?? lastAutopoieticSnapshot;
  if (organismSnapshot.temporal?.state.now) {
    temporalHistory.push(organismSnapshot.temporal.state.now);
    if (temporalHistory.length > 64) {
      temporalHistory.splice(0, temporalHistory.length - 64);
    }
  }
  ...
  lastOntogenesisVector = describeOntogenesisVector({
    ...
    autopoieticSnapshot: organismSnapshot.autopoietic,
    note: lastOntogenesisVector.note,
  });
  ...
  lastOntogenesisVector = {
    ...
    autopoietic: lastAutopoieticSnapshot ?? lastOntogenesisVector.autopoietic,
    ...
    temporal: organismSnapshot.temporal
      ? {
          tmi: organismSnapshot.temporal.timeCrystal.tmi.value,
          drift: organismSnapshot.temporal.timeCrystal.drift.value,
          alignment: organismSnapshot.temporal.timeCrystal.alignment.value,
          mode: organismSnapshot.temporal.decision.mode,
          warning: organismSnapshot.temporal.decision.warningLevel,
        }
      : lastOntogenesisVector.temporal,
  };
